...
def Customer_Registration(request):
    if request.POST:
        ...
        type = "Customer"
        qry = "insert into cust_reg(cname,address,pincode,gender,age,district,location,email,mobile,password) values('" + cname + "','" + address + \
...
def SignIn(request):
    ...
    if request.POST:
        try:
            ...
            email = request.POST.get("email")
            password = request.POST.get("password")
            c.execute("select * from login where username='" +
            ...
            if ds[2] == 'Admin':
                ...
                c.execute("select * from cust_reg where email='" +
                        email+"' and password='"+password+"'")
                ds = c.fetchone()
                ...
                c.execute("select * from club_reg where email='" +
                        email+"' and password='"+password+"'")
                ds = c.fetchone()
                request.session['cid'] = ds[0]
                return HttpResponseRedirect('/ClubHome/')
            elif ds[2] == 'Shop':
                c.execute("select * from shop_reg where email='" +
                        email+"' and password='"+password+"'")
                ds = c.fetchone()
                request.session['sid'] = ds[0]
                return HttpResponseRedirect('/ShopHome/')
            elif ds[2] == 'Mechanic':
                c.execute("select * from tblmechanic where mEmail='" +
                        email+"' and password='"+password+"'")
                ds = c.fetchone()
                request.session['mechid'] = ds[0]
                return HttpResponseRedirect('/MechHome/')
        except:
...
def Club_Registration(request):
    if request.POST:
        ...
        status = "Registered"
        type = "Club"
        qry = "insert into club_reg(clname,district,city,phone,email,reg_no,password,status) values('" + cname + "','" + \
            district + "','" + location + "','" + mobile + "','" + email + \
...
def Shop_Registration(request):
    if request.POST:
        ...
        regno = request.POST.get("regno")
        password = request.POST.get("Password")
        qry = "insert into shop_reg(shop,district,city,phone,email,reg_no,password,status) values('" + cname + "','" + \
...
def Contact(request):
    if request.POST:
        ...
        qry = "insert into feedback(`name`,`phone`,`email`,`msg`) values ('"+str(
            name)+"','"+str(phone)+"','"+str(email)+"','"+str(msg)+"')"
        c.execute(qry)
        db.commit()
...
def Admin_Add_Hospital(request):
    ...
    if request.POST:
        ...
        phone = request.POST.get("phone")
        email = request.POST.get("email")
        qry = "insert into hospital_reg(hname,address,district,location,phone,email) values('" + name + \
            "','" + adrs + "','" + district + "','" + \
...
def Admin_Add_Hotel(request):
    ...
    if request.POST:
        ...
        qry = "insert into hotel_reg(hotel,address,district,location,phone,email) values('" + \
            name + "','" + address + "','" + dist + "','" + \
            loc + "','" + pho + "','" + em + "')"
        c.execute(qry)
        db.commit()
...
def Admin_Add_Workshop(request):
    ...
    if request.POST:
        ...
        em = request.POST.get("email")
        qry = "insert into workshop_reg(wname,address,district,location,phone,email) values('" + \
            na + "','" + ad + "','" + di + "','" + lo + "','" + ph + "','" + em + "')"
        c.execute(qry)
        db.commit()
...
def Admin_View_Feedback(request):
    data = ""
    c.execute(
        "SELECT feedback.*, cust_reg.* FROM feedback INNER JOIN cust_reg on cust_reg.cid = feedback.userid")
    data = c.fetchall()
    if request.GET:
        a = request.GET.get('id')
...
def Admin_View_Club(request):
    ...
    if request.GET:
        ...
        c.execute("update club_reg set status = '" +
                  st+"' where clid = '"+cl+"'")
        db.commit()
...
def Club_Add_Notification(request):
    ...
    if request.POST:
        ...
        qry = "insert into notification(notification,tripdate,notdate,clid) values('" + \
            a + "','" + b + "','" + str(date) + "','" + str(clid) + "')"
        c.execute(qry)
        db.commit()
...
def Club_View_Request(request):
    clid = request.session['cid']
    c.execute("SELECT clubjoin_req.*, cust_reg.*,club_reg.clid,club_reg.clname FROM clubjoin_req INNER JOIN cust_reg ON clubjoin_req.uid=cust_reg.cid inner join club_reg on club_reg.clid=clubjoin_req.clid where club_reg.clid = '" +
              str(clid)+"' and clubjoin_req.status = 'Request'")
    data = c.fetchall()
    if request.GET:
        cl = request.GET.get('id')
        st = request.GET.get('st')
        c.execute("update clubjoin_req set status = '" +
                  st+"' where reid = '"+str(cl)+"'")
        db.commit()
...
def Club_Add_Vehicle(request):
    ...
    if request.POST:
        ...
        if request.POST and request.FILES:
            ...
            c.execute("insert into club_vehicle(clid,ve_name,cu_cap,reg_no,no_item,rent,img) values('" +
                      str(clid) + "','"+name+"','"+cap+"','"+regno+"','1','"+rent+"','"+path+"')")
            db.commit()
...
def Club_View_Booking(request):
    ...
    if request.GET:
        ...
        if st == 'Accept':
            c.execute("update vehicle_rent set status = '" +
                      st+"' where rent_id = '"+str(cl)+"'")
            db.commit()
...
def Customer_View_Profile(request):
    ...
    if request.session['uid']:
        ...
        if request.POST:
            ...
            c.execute("update cust_reg set mobile='"+Phn +
...
def Customer_Search_Club(request):
    ...
    if request.GET:
        ...
        date = datetime.date.today()
        status = "Request"
        c.execute("insert into clubjoin_req(clid,uid,date,status) values('" +
                  str(a)+"','"+str(uid)+"','"+str(date)+"','"+status+"')")
        db.commit()
...
def CustomerRentBikeRequest(request):
    ...
    if 'sub' in request.POST:
        query="select count(*) from `vehicle_rent` where `custid`='"+str(uid)+"' and status='Request'"
        c.execute(query)
        data1=c.fetchone()
        if data1[0]==0:
            pd = request.POST.get("pdate")
            ...
            data = c.fetchone()
            clubid = data[0]
            c.execute("insert into vehicle_rent(veh_id,custid,clubid,pickup_date,return_date,cvrent_type,rent_unit,rent_days,amount,status) values('" +
                    str(vid)+"','"+str(cus)+"','"+str(clubid)+"','"+str(pd)+"','"+str(rd)+"','"+str(rtp)+"','1','"+str(td)+"','"+str(totam)+"','Request')")
            db.commit()
        else:
            msg="Already Booked"
        print(msg)    
    return render(request, 'CustomerRentBikeRequest.html', {"total_amount": total_amount, "days": days, "pd": pd, "rd": rd, "td": td,"msg":msg})
...
def subcatforbike(request):
    ...
    # print("SELECT club_vehicle.*, club_reg.* FROM club_vehicle INNER JOIN club_reg ON club_vehicle.clid=club_reg.clid  where club_reg.district='" + str(catid)+"'")
    data = c.fetchall()
    return HttpResponse(json.dumps(data), content_type="application/json")
...
def CustomerOrderProduct(Request):
    ...
    merid = data[0][8]
    price = data[0][6]
    c.execute("insert into cart (cid,fid,pid,amount,qty)values('" +
              str(cid)+"','"+str(merid)+"','"+str(pid)+"','"+str(price)+"','1')")
    db.commit()
    t = "select * from categories"
    c.execute(t)
    data1 = c.fetchall()
    ...
    if(Request.POST):
        ...
        for d3 in data3:
            ...
            c.execute("insert into customer_order (uid,pid,p_price,p_qty)values('" +
                      str(custid)+"','"+str(proid)+"','"+str(amot)+"','"+str(quty)+"')")
            db.commit()
...
def MechSignUp(request):
    if request.POST:
        ...
        mexp = request.POST.get("mexp")
        # gender = request.POST.get("gender")
        # age = request.POST.get("age")
        mdist = request.POST.get("mdist")
        mloc = request.POST.get("mloc")
        email = request.POST.get("email")
        phone = request.POST.get("phone")
        password = request.POST.get("Password")
        type = "Mechanic"
        qry = "INSERT INTO tblmechanic (`mName`,`mAddress`,`mContact`,`mDistrict`,`mEmail`,`mExp`,`place`,`password`) VALUES('"+str(
            mname)+"','"+str(maddress)+"','"+str(phone)+"','"+str(mdist)+"','"+str(email)+"','"+str(mexp)+"','"+str(mloc)+"','"+str(password)+"')"
        qr = "insert into login values('" + email + \
            "','" + password + "','" + type + "')"
        c.execute(qry)
        c.execute(qr)
...
def Mch_Home(request):
    return render(request, 'Mech Home.html')


def user_view_mechanic(request):
    data = ""
    c.execute("SELECT * FROM tblmechanic")
    data = c.fetchall()
    return render(request, "UserViewMechanic.html", {"data": data})


def work_request(request):
    uid = request.session['uid']
    mechid = request.GET.get('id')
    today = date.today()
    if request.POST:
        desc = request.POST.get('desc')
        query = "INSERT INTO workrequest(`mid`,`uid`,`requestdate`,`desc`,`status`) VALUES('"+str(
            mechid)+"','"+str(uid)+"','"+str(today)+"','"+str(desc)+"','requested')"
        c.execute(query)
        print(query)
        db.commit()
    return render(request, "work_request.html")


def customer_view_status(request):
    uid = request.session['uid']
    data = ""
    c.execute("SELECT tblmechanic.`mName`,`mEmail`,`workrequest`.* FROM tblmechanic,`workrequest` WHERE `tblmechanic`.`mid`=`workrequest`.`mid` AND `workrequest`.uid='"+str(uid)+"'")
    data = c.fetchall()
    # SELECT tblmechanic.`mName`,`mEmail`,`workrequest`.* FROM tblmechanic,`workrequest` WHERE `tblmechanic`.`mid`=`workrequest`.`mid` AND `workrequest`.uid='"+uid+"'
    return render(request, "customer_view_status.html", {"data": data})


def Mech_Requests(request):
    mid = request.session['mechid']
    data = ""
    c.execute("SELECT cust_reg.`cname`,`email`,`mobile`,`workrequest`.* FROM cust_reg,`workrequest` WHERE `cust_reg`.`cid`=`workrequest`.`uid` AND `workrequest`.mid='"+str(mid)+"'and workrequest.status='requested'")
    data = c.fetchall()
    data1 = ""
    c.execute("SELECT cust_reg.`cname`,`email`,`mobile`,`workrequest`.* FROM cust_reg,`workrequest` WHERE `cust_reg`.`cid`=`workrequest`.`uid` AND `workrequest`.mid='"+str(mid)+"'and workrequest.status='approved'")
    data1 = c.fetchall()
    return render(request, "Mech_Requests.html",{"data":data,"data1":data1})

def statusAction(request):
    status=request.GET.get('status')
    requestId=request.GET.get('id')
    
    query="UPDATE workrequest SET `status`='"+str(status)+"' WHERE  requestid='"+str(requestId)+"'"
    c.execute(query)
    db.commit()
    return  HttpResponseRedirect("/Mech_Requests")


def Mech_Requests_Approved(request):
    mid = request.session['mechid']
    data1 = ""
    c.execute("SELECT cust_reg.`cname`,`email`,`workrequest`.* FROM cust_reg,`workrequest` WHERE `cust_reg`.`cid`=`workrequest`.`uid` AND `workrequest`.mid='"+str(mid)+"'and workrequest.status='approved'")
    data1 = c.fetchall()
    return render(request, "Mech_Requests.html",{"data1":data1})

def customer_join_status(request):
    uid = request.session['uid']
    c.execute("SELECT clubjoin_req.*, cust_reg.*,club_reg.clid,club_reg.clname FROM clubjoin_req INNER JOIN cust_reg ON clubjoin_req.uid=cust_reg.cid INNER JOIN club_reg ON club_reg.clid=clubjoin_req.clid WHERE cust_reg.`cid` = '"+str(uid)+"' AND clubjoin_req.status = 'Accept'")
    data = c.fetchall()
    
    return render(request,'user join status.html',{"data":data})
